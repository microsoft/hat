...
def generate_arg_sets_for_func(func: hat_file.Function, input_sets_minimum_size_MB: int = 0, num_additional: int = 0, dyn_func_shape_fn: Callable[[FunctionInfo], List[List[int]]]=None):
    def default_dyn_func_shape_fn(func: FunctionInfo) -> List[List[int]]:
    ...
    if any(map(lambda p: not p.is_constant_shaped, func_info.arguments)):
        ...
        while param_idx < len(parameters):
            ...
            for dyn_dim_idx, dyn_dim in dyn_dimensions:
                param_idx += 1
                dim_names_to_values[dyn_dim] = ArgValue(parameters[param_idx], np.array([numerical_shape[dyn_dim_idx]], dtype=np.uint32))
            
            param_idx += 1
            shape_idx += 1

    else:
        numerical_shapes = [p.shape for p in func.arguments]
        _expected_shape = default_dyn_func_shape_fn(func)
        assert numerical_shapes == _expected_shape
