...
class HATPackage:
    def __init__(self, hat_file_path):
        ...
        self.link_target_path = os.path.join(
            os.path.split(self.hat_file_path)[0],
            self.hat_file.dependencies.link_target)
    ...
    def get_functions_for_target(self,
                                 os: str,
                                 arch: str,
...
def _make_cpu_func(shared_lib: ctypes.CDLL, function_name: str,
                   arg_infos: List[Parameter]):
    arg_infos = [d for d in arg_infos]
...
def _make_device_func(func_runtime: str, hat_dir_path: str, func: Function):
    if func_runtime == "CUDA":
        ...
        return cuda_loader.create_loader_for_device_function(
            func, hat_dir_path)
    elif func_runtime == "ROCM":
        from . import rocm_loader
        return rocm_loader.create_loader_for_device_function(
            func, hat_dir_path)


def hat_package_to_func_dict(
        hat_pkg: HATPackage) -> Union[AttributeDict, None]:
    ...
    if os.path.isfile(hat_pkg.link_target_path):
        ...
        if extension and extension not in supported_extensions:
            ...
            raise RuntimeError(
                f"Unsupported HAT library extension: {extension}")
    ...
    for func_name, func_desc in hat_pkg.hat_file.function_map.items():
        ...
        if not launches and shared_lib:
            ...
            func_dict[func_name] = _make_cpu_func(hat_library, func_desc.name,
                                                  func_desc.arguments)
        else:
            ...
            func_dict[func_name] = _make_device_func(func_runtime=func_runtime,
                                                     hat_dir_path=hat_dir_path,
                                                     func=device_func)
