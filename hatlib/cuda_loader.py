#!/usr/bin/env LD_LIBRARY_PATH=/usr/lib/wsl/lib:$LD_LIBRARY_PATH python3
...
def foo():
    ...
    prog = Program(src=src, name=PROG_NAME,
    ...
    ASSERT_DRV(err)
    err, kernel = cuda.cuModuleGetFunction(module, b"NestFunction_0")
    ASSERT_DRV(err)
    ...
    N = 65536
    bufferSize = N * np.array([1.], dtype=np.float32).itemsize

    hArg1 = np.random.rand(N).astype(dtype=np.float32)
    hArg2 = np.random.rand(N).astype(dtype=np.float32)
    hArg3 = np.zeros(N).astype(dtype=np.float32)

    err, dArg1 = cuda.cuMemAlloc(bufferSize)
    ASSERT_DRV(err)
    err, dArg2 = cuda.cuMemAlloc(bufferSize)
    ASSERT_DRV(err)
    err, dArg3 = cuda.cuMemAlloc(bufferSize)
    ASSERT_DRV(err)

    err, stream = cuda.cuStreamCreate(0)
    ASSERT_DRV(err)

    # err, = cuda.cuMemcpyHtoDAsync(dArg1, hArg1.ctypes.data, bufferSize, stream)
    # ASSERT_DRV(err)
    # err, = cuda.cuMemcpyHtoDAsync(dArg2, hArg2.ctypes.data, bufferSize, stream)
    # ASSERT_DRV(err)

    err, = cuda.cuMemcpyHtoD(dArg1, hArg1.ctypes.data, bufferSize)
    ASSERT_DRV(err)
    err, = cuda.cuMemcpyHtoD(dArg2, hArg2.ctypes.data, bufferSize)
    ASSERT_DRV(err)

    # The following code example is not intuitive
    # Subject to change in a future release
    dArg1Ptr = np.array([int(dArg1)], dtype=np.uint64)
    dArg2Ptr = np.array([int(dArg2)], dtype=np.uint64)
    dArg3Ptr = np.array([int(dArg3)], dtype=np.uint64)

    args = [dArg1Ptr, dArg2Ptr, dArg3Ptr]
    ...
    # err, = cuda.cuMemcpyDtoHAsync(hArg3.ctypes.data, dArg3, bufferSize, stream)
    # ASSERT_DRV(err)
    err, = cuda.cuStreamSynchronize(stream)
    ASSERT_DRV(err)
    err, = cuda.cuMemcpyDtoH(hArg3.ctypes.data, dArg3, bufferSize)
    ASSERT_DRV(err)

    hArg3Expected = hArg1 + hArg2
    if not np.allclose(hArg3, hArg3Expected):
